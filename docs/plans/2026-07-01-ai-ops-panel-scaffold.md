...
**架构：** 开发阶段前端和后端分别运行；Docker 构建阶段把 React/Vite 前端编译成静态资源，并由 FastAPI 后端统一托管。后端负责登录、配置、服务器记录、SSH 连接测试、Web 终端代理、部署计划校验、命令安全检查和部署执行。第一版保持单容器友好，使用 SQLite 存储，并保留迁移到 PostgreSQL 和后台任务队列的空间。
...
2. 应用启动时初始化 SQLite 表。
3. 保持 API 端点简洁，并通过服务层承接 SSH 连接测试、资源快照和受控命令执行。
...
2. 添加服务器列表、AI 设置、部署流程、Web 终端和历史记录等面板区域。
3. 添加类型检查和构建命令。
